...
async function runTestSession(session: TestSession): Promise<TestResult> {
  ...
  try {
    ...
    const response = await axios.post(
      `${PRODUCTION_URL}/api/ide/code-edit`,
      {
    ...
    const userWouldAccept = simulateUserAcceptance(
      jazzAnalysis,
      response.data,
      session.expectedComplexity
    );
...
function simulateUserAcceptance(
  jazz: JazzAnalysis,
  response: any,
  ...
  if (jazz.trust >= 0.75) {
    ...
    if (expectedComplexity === 'simple' && codeLength < 1000) return true;
    if (expectedComplexity === 'medium' && codeLength < 2000) return true;
    if (expectedComplexity === 'complex') return true;
  }
...
function analyzeResults(results: TestResult[]): any {
  const analysis = {
    overall: {
    ...
    suggestions: {
      ...
      topPatterns: [] as { pattern: string; count: number; examples: string[] }[]
    },
    failureModes: [] as { mode: string; sessions: number[]; avgTrust: number }[],
    lowTrustSessions: [] as TestResult[],
    highTrustSessions: [] as TestResult[]
  };
  ...
  const suggestionKeywords = new Map<string, { count: number; examples: Set<string> }>();
  ...
  const failureModeMap = new Map<string, { sessions: number[]; trusts: number[] }>();
...
function identifyFailureMode(result: TestResult): string {
  if (result.failureMode) return `API Error: ${result.failureMode}`;
  
  const jazz = result.jazzAnalysis;
  
  if (jazz.trust < 0.70) return 'Low Trust (τ < 0.70)';
  if (jazz.voice < 0.75) return 'Low Voice (unclear communication)';
  if (jazz.choice < 0.75) return 'Low Choice (limited options)';
  if (jazz.transparency < 0.75) return 'Low Transparency (unclear reasoning)';
  if (jazz.suggestions.length < 2) return 'Insufficient Suggestions';
  
  return 'User Preference Mismatch';
}

function generateRoadmap(analysis: any, results: TestResult[]): string {
...
${analysis.suggestions.topPatterns.map((p: any, i: number) => `
### ${i + 1}. ${p.pattern.toUpperCase()} (${p.count} occurrences)

**Examples:**
${p.examples.map((ex: string) => `- "${ex}"`).join('\n')}
...
${analysis.failureModes.length > 0 ? analysis.failureModes.map((fm: any) => `
### ${fm.mode}
...
${r.jazzAnalysis.suggestions.map((s: string) => `  - ${s}`).join('\n')}
`).join('\n')}
...
async function main() {
  ...
  // Save raw data
  fs.writeFileSync(
    '/home/ubuntu/vctt_agi_engine/jazz_test_results.json',
