...
export class VCTTEngineService {
  ...
  async processStep(sessionId: string, input: string): Promise<any> {
    ...
    this.logger.log('🍄 Running pre-jam truth sweep with Grok-4-Fast-Reasoning...');
    ...
    if (bandJamResults && bandJamResults.weights) {
      ...
      this.trackContribution(sessionId, 'verification', 'grok-4-fast-reasoning', !!bandJamResults.results.verification, false);
